...
public class PhotoService extends ServiceImpl<PhotoMapper, PhotoEntity> {
    ...
    @Value("${aliyun.oss.bucket:ringstory-photos}") private String bucket;

    @Transactional
    public PhotoEntity uploadPhoto(Long familyId, Long uploaderId, MultipartFile file) {
        String ossKey = String.format("%d/%s/%s", familyId, LocalDateTime.now().toLocalDate(), IdUtil.fastSimpleUUID());
    ...
    public boolean toggleLike(Long photoId, Long userId) {
        LikeEntity existing = likeMapper.selectOne(
            new com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper<LikeEntity>()
                .eq(LikeEntity::getPhotoId, photoId)
                .eq(LikeEntity::getUserId, userId));
        if (existing != null) {
            likeMapper.deleteById(existing.getId());
            lambdaUpdate().eq(PhotoEntity::getId, photoId).setSql("like_count = like_count - 1").update();
            return false;
        }
        LikeEntity like = new LikeEntity();
        like.setPhotoId(photoId); like.setUserId(userId);
    ...
    public CommentEntity addComment(Long photoId, Long authorId, String content, Long parentId) {
        ...
        comment.setPhotoId(photoId); comment.setAuthorId(authorId);
        comment.setContent(content); comment.setParentId(parentId);
        commentMapper.insert(comment);
        lambdaUpdate().eq(PhotoEntity::getId, photoId).setSql("comment_count = comment_count + 1").update();
        return comment;
    }
}
