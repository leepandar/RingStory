...
public class SearchService {
     ...
     * 搜索照片
     */
    public List<Map<String, Object>> searchPhotos(Long familyId, String keyword,
                                                   Long personId, String location,
                                                   String dateFrom, String dateTo) {
        StringBuilder sql = new StringBuilder(
                "SELECT p.id, p.oss_key, p.shoot_time, p.original_name, "
                        + "pn.content as note_content "
                        + "FROM t_photo_2026 p LEFT JOIN t_photo_note pn ON p.id = pn.photo_id "
